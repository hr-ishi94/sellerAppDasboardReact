...
import { Line,XAxis,YAxis,Tooltip ,LineChart, CartesianGrid, Legend} from 'recharts';
...
const SalesOverviewChart = ({overview}) => {
  ...
  <div >
    ...
    <LineChart width={866} height={240} data={data} className="mt-2">
  {/* Use rectangles for alternating vertical background colors */}
  {data.map((_, index) => (
    <rect
      key={index}
      x={index * (882 / data.length) + 6}  // Dynamically calculate width based on the number of data points
      y="6"
      width={882 / data.length} // Each section takes an equal part of the chart's width
      height="200" // Full height of the chart
